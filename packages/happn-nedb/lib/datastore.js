 ...
 * @param {Boolean} options.timestampData Optional, defaults to false. If set to true, created and modified will be created and populated automatically (if not specified by user)
...
Datastore.prototype.prepareDocumentForInsertion = function (newDoc) {
  ...
  if (util.isArray(newDoc)) {
    ...
    var now = Date.now();
    if (this.timestampData && preparedDoc.created === undefined) {
      preparedDoc.created = now;
    }
    if (this.timestampData && preparedDoc.modified === undefined) {
      preparedDoc.modified = now;
    }
      ...
      self.getCandidates(query, function (err, candidates) {
          ...
          for (i = 0; i < candidates.length; i += 1) {
            if (model.match(candidates[i], query) && (multi || numReplaced === 0)) {
              ...
              if (self.timestampData) {
                created = candidates[i].created;
              }
              modifiedDoc = model.modify(candidates[i], updateQuery);
              if (self.timestampData) {
                modifiedDoc.created = created;
                modifiedDoc.modified = Date.now();
              }
        ...
        self.persistence.persistNewState(updatedDocs, function (err) {
          ...
          updatedDocs.forEach(function (doc) {
            updatedMetaDC.push(self.__meta(doc));
          });
          if (!multi) {
            updatedMetaDC = updatedMetaDC[0];
          }

          if (!options.returnUpdatedDocs) {
            return callback(null, numReplaced, undefined, undefined, updatedMetaDC);
          } else {
            var updatedDocsDC = [];
            updatedDocs.forEach(function (doc) {
            ...
            return callback(null, numReplaced, updatedDocsDC, undefined, updatedMetaDC);
          }
        });
      });
    },
...
Datastore.prototype.__meta = function (doc) {
  return { created: doc.created, modified: doc.modified, _id: doc._id };
};
