...
describe('Database', function() {
  ...
  describe('Insert', function() {
    ...
    it('If timestampData option is set, a created field is added and persisted', function(done) {
      ...
      d.find({}, function(err, docs) {
        ...
        d.insert(newDoc, function(err, insertedDoc) {
          ...
          assert.isBelow(
            Math.abs(new Date(insertedDoc.created).getTime() - beginning),
    ...
    it("If timestampData is set but created is specified by user, don't change it", function(done) {
      var newDoc = { hello: 'world', created: new Date(234) },
        beginning = Date.now();
      d = new Datastore({ filename: testDb, timestampData: true, autoload: true });
      d.insert(newDoc, function(err, insertedDoc) {
        Object.keys(insertedDoc).length.should.equal(4);
    ...
    it("If timestampData is set but modified is specified by user, don't change it", function(done) {
      var newDoc = { hello: 'world', modified: new Date(234) },
        beginning = Date.now();
      d = new Datastore({ filename: testDb, timestampData: true, autoload: true });
      d.insert(newDoc, function(err, insertedDoc) {
        Object.keys(insertedDoc).length.should.equal(4);
    ...
    it('If the callback throws an uncaught exception, do not catch it inside findOne, this is userspace concern', function(done) {
      var tryCount = 0,
        currentUncaughtExceptionHandlers = process.listeners('unhandledRejection'),
        i;
  ...
  describe('Update', function() {
    ...
    describe('Upserts', function() {
      it('Can perform upserts if needed', function(done) {
        d.update({ impossible: 'db is empty anyway' }, { newDoc: true }, {}, function(
          err,
          nr,
          upsert
        ) {
      ...
      it('Performing upsert with badly formatted fields yields a standard error not an exception', function(done) {
        d.update({ _id: '1234' }, { $set: { $$badfield: 5 } }, { upsert: true }, function(
          err,
          doc
        ) {
          assert.isDefined(err);
    ...
    it('Can upsert a document even with modifiers', function(done) {
      d.update({ bloup: 'blap' }, { $set: { hello: 'world' } }, { upsert: true }, function(
        err,
        nr,
        newDoc
      ) {
                ...
                d.find({}, function(err, docs) {
                  ...
                  d.loadDatabase(function(err) {
                    ...
                    d.find({}, function(err, docs) {
                      ...
                      docs.length.should.equal(3);
                      _.isEqual(docs[0], { _id: doc1._id, a: 1, hello: 'changed' }).should.equal(
                        true
                      );
                      _.isEqual(docs[1], { _id: doc2._id, a: 2, hello: 'changed' }).should.equal(
                        true
                      );
                      _.isEqual(docs[2], { _id: doc3._id, a: 5, hello: 'pluton' }).should.equal(
                        true
                      );
      ...
      d.insert({ a: 4 }, function(err, doc1) {
        d.insert({ a: 5 }, function(err, doc2) {
          d.insert({ a: 'abc' }, function(err, doc3) {
            ...
            d.update({ a: { $in: [4, 5, 'abc'] } }, { $inc: { a: 10 } }, { multi: true }, function(
              err
            ) {
              assert.isDefined(err);
    ...
    it('If an index constraint is violated by an update, all changes should be rolled back', function(done) {
      ...
      d.insert({ a: 4 }, function(err, doc1) {
        d.insert({ a: 5 }, function(err, doc2) {
          // With this query, candidates are always returned in the order 4, 5, 'abc' so it's always the last one which fails
          d.update({ a: { $in: [4, 5, 'abc'] } }, { $set: { a: 10 } }, { multi: true }, function(
            err
          ) {
            assert.isDefined(err);
    ...
    it('If options.returnUpdatedDocs is true, return all matched docs', function(done) {
      d.insert([{ a: 4 }, { a: 5 }, { a: 6 }], function(err, docs) {
        ...
        d.update({ a: 7 }, { $set: { u: 1 } }, { multi: true, returnUpdatedDocs: true }, function(
          err,
          num,
          updatedDocs
        ) {
          ...
          d.update({ a: 5 }, { $set: { u: 2 } }, { multi: true, returnUpdatedDocs: true }, function(
            err,
            num,
            updatedDocs
          ) {
    ...
    describe('Callback signature', function() {
      it('Regular update, multi false', function(done) {
        ...
        d.update({ a: 1 }, { $set: { b: 20 } }, {}, function(
          ...
          d.update({ a: 1 }, { $set: { b: 21 } }, { returnUpdatedDocs: true }, function(
            err,
            numAffected,
            affectedDocuments,
            upsert,
      ...
      it('Regular update, multi true', function(done) {
        ...
        d.update({}, { $set: { b: 20 } }, { multi: true }, function(
          ...
          // returnUpdatedDocs set to true
          d.update({}, { $set: { b: 21 } }, { multi: true, returnUpdatedDocs: true }, function(
            err,
            numAffected,
            affectedDocuments,
            upsert,
            ...
            db.ensureIndex({ fieldName: 'planet', unique: true, sparse: false }, function(err) {
              ...
              db.insert({ planet: 'Jupiter' }, function(err) {
                ...
                db.loadDatabase(function(err) {
                  ...
                  db.ensureIndex({ fieldName: 'bloup', unique: false, sparse: true }, function(
                    err
                  ) {
                    assert.isNull(err);
