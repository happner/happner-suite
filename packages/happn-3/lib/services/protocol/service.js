...
function __processSinglePath(transformed, callback) {
  this.happn.services.security.processAuthorize(transformed, (e, authorized) => {
    ...
    if (authorized.request.action === 'on') {
      return this.happn.services.subscription.processSubscribe(authorized, function (e, result) {
        if (e) return callback(e);
        callback(null, result);
      });
    }
    ...
    if (authorized.request.action === 'off')
      return this.happn.services.subscription.processUnsubscribe(authorized, function (e, result) {
        if (e) return callback(e);
        callback(null, result);
      });
...
function __getProtocol(message) {
  const protocol = this.config.protocols[message.session.protocol];
  if (protocol != null) {
    return protocol;
  }
  if (
    message.session.protocol.indexOf('1') === 0 ||
    message.session.protocol.indexOf('happn') === 0
  ) {
    //backward compatability
    return this.config.protocols.happn_1;
  }
  return null;
}
...
function processMessageOut(message, callback) {
  try {
    const protocol = this.__getProtocol(message);
    const publication = protocol.transformOut(message);
