module.exports = class AuthProvider {
  ...
  login(credentials, sessionId, request, callback) {
    ...
    if (credentials.username === '_ANONYMOUS') {
      if (!this.config.allowAnonymousAccess) {
        return callback(
          this.happn.services.error.InvalidCredentialsError('Anonymous access is disabled')
        );
      }
  ...
  __providerTokenLogin(username, credentials, callback) {
    return this.accessDenied('__providerTokenLogin not implemented.', callback);
  }
  ...
  __providerCredsLogin(credentials, sessionId, callback) {
    return this.accessDenied('__providerCredsLogin not implemented.', callback);
  }
