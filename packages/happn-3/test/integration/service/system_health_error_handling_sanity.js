  ...
  function () {
    ...
    it('logs errors via the error service, ensures system health changes as required', function (done) {
      ...
      expect(serviceInstance.services.system.__stats.HEALTH.STATUS).to.be(
        constants.SYSTEM_HEALTH.EXCELLENT
      );

      serviceInstance.services.error.handleSystem(e, 'test', constants.ERROR_SEVERITY.LOW);

      expect(serviceInstance.services.system.__stats.HEALTH.STATUS).to.be(
        constants.SYSTEM_HEALTH.FAIR
      );

      expect(
        serviceInstance.services.system.__stats.HEALTH.BY_SEVERITY[constants.ERROR_SEVERITY.LOW]
      ).to.be(1);
      ...
      expect(serviceInstance.services.system.__stats.HEALTH.STATUS).to.be(
        constants.SYSTEM_HEALTH.TAKING_STRAIN
      );

      expect(
        serviceInstance.services.system.__stats.HEALTH.BY_SEVERITY[constants.ERROR_SEVERITY.MEDIUM]
      ).to.be(1);
      ...
      expect(serviceInstance.services.system.__stats.HEALTH.lastError.message).to.be(
        'Error: TEST ERROR'
      );

      expect(serviceInstance.services.system.__stats.HEALTH.lastError.severity).to.be(
        constants.ERROR_SEVERITY.FATAL
      );
        ...
        .then(function (client) {
          ...
          client.on(
            ...
            function (e) {
              ...
              serviceInstance.services.subscription.addListener =
                serviceInstance.services.subscription.__oldAddListener;
              ...
              client.set(
                ...
                function (e) {
                  ...
                  setTimeout(() => {
                    ...
                    expect(stats.HEALTH.lastError.severity).to.be(1);
                    serviceInstance.services.publisher.processPublish =
                      serviceInstance.services.publisher.__oldPublishMessage;
                    done();
                  }, 2000);
                }
