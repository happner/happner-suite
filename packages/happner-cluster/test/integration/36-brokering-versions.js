...
require('../_lib/test-helper').describe({ timeout: 120e3 }, (test) => {
  ...
  context('brokering latest versions', function () {
    it('can broker with version "*", no methods defined, lower version joins first', async function () {
      ...
      version = thisClient.exchange.remoteComponent.getVersion();
      let version2 = thisClient.exchange.remoteComponent.getVersion();
      await Promise.all([version, version2])
    ...
    it('can broker with version "*", no methods defined, higher version joins first', async function () {
      ...
      test
        .expect(version)
        .to.eql({ mesh: 'MESH_3', version: '3.4.5', component: 'remoteComponent2' });
