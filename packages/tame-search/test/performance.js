require('happn-commons-test').describe({ timeout: 5000 }, (test) => {
  ...
  const util = require('./__fixtures/util');
  const uuid = test.uuid;
  const TameSearch = require('..');
  ...
  it(
    'creates ' +
      SUBSCRIPTION_COUNT +
      ' random paths, then does sparse wildcard searches ' +
      SEARCH_COUNT +
      ' times, this should be faster than the test preceding it',
    function (done) {
      const subscribers = [...Array(SUBSCRIBER_COUNT)].map((_item) => {
        return uuid.v1();
      });
  ...
  it(
    'creates ' +
      ...
      ' random subscriptions, for ' +
      SUBSCRIBER_COUNT +
      ' subscriber keys - we ensure unsubscribing from all is performant',
    function (done) {
      const subscribers = [...Array(SUBSCRIBER_COUNT)].map((_item) => {
        return uuid.v1();
      });
      ...
      console.log(
        'unsubscribed from all in ' +
          ((Date.now() - startedUnsubscribing) / 1000).toString() +
          ' seconds'
      );
