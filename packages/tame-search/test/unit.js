...
describe('tame-search unit', function () {
  ...
  it('tests getting wildcard combinations', function (done) {
    ...
    expect(
      tameSearch.getWildcardCombinations(['my', 'test', 'path', '1'], '/my/test/path/1')
    ).to.eql([]);
    ...
    expect(
      tameSearch.getWildcardCombinations(['my', 'test', 'path', '1'], '/my/test/path/1')
    ).to.eql(['/*/test/path/1', '/my/test/path/*']);
  ...
  it('tests the search getSubscriptionsByTopic update function', function (done) {
    ...
    expect(tameSearch.getSubscriptionsByTopic('/my/test/path/*', { filter: { test: 1 } })).to.eql([
      { test: 1, custom: 0, subscriberKey: 'test-subscriber' },
    ]);
    done();
  });
  ...
  it('tests the search subscriptions we are able to update some data', function (done) {
    ...
    tameSearch.subscribe('test-subscriber', '/my/test/path/*', { test: 2, custom: 0 });
    tameSearch.subscribe('test-subscriber', '/my/test/path/*', { test: 3, custom: 0 });
    var subscription = tameSearch.getSubscriptionsByTopic('/my/test/path/*', {
      filter: { test: 1 },
    })[0];
    subscription.custom++;
    subscription.custom++;
    var modified = tameSearch.getSubscriptionsByTopic('/my/test/path/*', {
      filter: { test: 1 },
    })[0];
