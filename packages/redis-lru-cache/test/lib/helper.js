...
TestCluster.prototype.verifyConsistency = function (callback) {
  ...
  async.eachSeries(
    idDown,
    function (idDownItem, idDownItemCB) {
      async.eachSeries(
        idUp,
        function (idUpItem, idUpItemCB) {
          if (idDownItem != idUpItem) {
            ...
            async.eachSeries(
              _this.__consistencyTotals[idUpItem],
              function (logItem, logItemCB) {
                ...
                _this.__clients[idDownItem].remote.on('message', function (serialized) {
                  ...
                  if (
                    deserialized.event == 'cache-get-complete' &&
                    ...
                    if (deduplicateMessages[deserialized.message.message.key]) {
                      ...
                      if (
                        deserialized.message.data == null ||
                        deserialized.message.data.test != logItem.value.test
                      )
                        misses++;
                ...
                _this.__clients[idDownItem].remote.send(
                  JSON.stringify({ type: 'doGet', key: logItem.key })
                );
              },
              idUpItemCB
            );
  ...
  async.eachSeries(
    ...
    function (pair, pairCB) {
      ...
      _this.__clients[idDownItem].remote.on('message', function (serialized) {
        ...
        if (
          deserialized.event == 'delete-all-failed' &&
        ...
        if (
          deserialized.event == 'delete-all-complete' &&
          deserialized.message.message.key === idUpItem
        ) {
          clearTimeout(doDelTimeout);

          setTimeout(function () {
            _this.verifyDelConsistencyItemsDontExist(
              idUpItem,
              toDeleteItems,
              function (e, vfound, vfailed) {
      ...
      _this.__clients[idDownItem].remote.send(
        JSON.stringify({ type: 'doDeleteAll', key: idUpItem, items: toDeleteItems })
      );
    },
    function (e) {
      if (e) return callback(e);
...
TestCluster.prototype.start = function (callback) {
  ...
  async.each(
    Object.keys(this.__clients),
    function (clientId, clientCB) {
      try {
...
TestHelper.prototype.getCluster = function (
  testId,
  size,
  init,
  clientSize,
