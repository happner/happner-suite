require('../../__fixtures/utils/test_helper').describe({ timeout: 120e3 }, (test) => {
  var serviceInstance;
  var clientInstance = new test.Mesh.MeshClient({ secure: true });
    ...
    setTimeout(function () {
      stopService(function (e) {
        ...
        test.Mesh.create(config, function (err, instance) {
          ...
          clientInstance = new test.Mesh.MeshClient({ secure: true });
  ...
  it('tests inserting inbound and outbound layers', function (callback) {
    ...
    getService(inboundLayers, outboundLayers, function (e) {
      ...
      clientInstance.data.on(
        '/did/both',
        function () {
          test.expect(layerLog1.length > 0).to.be(true);
          test.expect(layerLog2.length > 0).to.be(true);
          test.expect(layerLog3.length > 0).to.be(true);
          test.expect(layerLog4.length > 0).to.be(true);
