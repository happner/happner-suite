require('../../__fixtures/utils/test_helper').describe({ timeout: 30e3 }, (test) => {
  const Mesh = require('../../..');
  const dbFileName = test.newTempFilename('loki');

  global.TESTING_7 = true;

  var config = {
    secure: true,
    happn: {
      persist: true,
      filename: dbFileName,
      adminPassword: 'TEST7',
    },
    components: {
      data: {},
    },
  };

  var unpersistedConfig = {
    port: 8888,
    secure: true,
    happn: {
      persist: false,
    },
  };

  after(function (done) {
    var _this = this;
    test.commons.fs.unlink(dbFileName, function (e) {
      if (e) return done(e);
      _this.mesh
        .stop({ reconnect: false })
        .then(function () {
          _this.unpersistedMesh.stop({ reconnect: false }, done);
        })
        .catch(done);
    });
  });

  var meshName;
  var meshPublicKey;
  var unpersistedMeshName;
  var unpersistedMeshPublicKey;

  before(function (done) {
    var _this = this;

    Mesh.create(config)
      .then(function (mesh) {
        _this.mesh = mesh;
        meshName = mesh._mesh.config.name;
        meshPublicKey = mesh._mesh.happn.server.services.security._keyPair.publicKey;

        Mesh.create(unpersistedConfig)
          .then(function (unpersistedMesh) {
            _this.unpersistedMesh = unpersistedMesh;
            unpersistedMeshName = unpersistedMesh._mesh.config.name;
            unpersistedMeshPublicKey =
              unpersistedMesh._mesh.happn.server.services.security._keyPair.publicKey;

            // console.log('names:::', meshName, unpersistedMeshName);

            done();
          })
          .catch(done);
      })
      .catch(done);
  });

  it('restarts the mesh, ensures the keypair', function (done) {
    this.mesh.stop({ reconnect: false }, (e) => {
      if (e) return done(e);
      Mesh.create(config)
        .then((mesh) => {
          this.mesh = mesh;
          test
            .expect(mesh._mesh.happn.server.services.security._keyPair.publicKey.toString())
            .to.be(meshPublicKey.toString());
          test.expect(mesh._mesh.happn.server.services.security._keyPair.publicKey).to.not.be(null);
          test
            .expect(mesh._mesh.happn.server.services.security._keyPair.publicKey)
            .to.not.be(undefined);
          done();
        })
        .catch(done);
    });
  });

  it('restarts the unpersisted mesh, ensures the keypair is different', function (done) {
    var _this = this;

    _this.unpersistedMesh.stop({ reconnect: false }, function (e) {
      if (e) return done(e);

      Mesh.create(unpersistedConfig)

        .then(function (unpersistedMesh) {
          _this.unpersistedMesh = unpersistedMesh;

          test
            .expect(
              unpersistedMesh._mesh.happn.server.services.security._keyPair.publicKey.toString()
            )
            .to.not.be(unpersistedMeshPublicKey.toString());
          test
            .expect(unpersistedMesh._mesh.happn.server.services.security._keyPair.publicKey)
            .to.not.be(null);
          test
            .expect(unpersistedMesh._mesh.happn.server.services.security._keyPair.publicKey)
            .to.not.be(undefined);
          test.expect(unpersistedMeshPublicKey).to.not.be(null);
          test.expect(unpersistedMeshPublicKey).to.not.be(undefined);

          done();
        })

        .catch(done);
    });
  });

  it('restarts the mesh, ensures the name', function (done) {
    var _this = this;

    _this.mesh.stop({ reconnect: false }, function (e) {
      if (e) return done(e);

      //so we need to check that we are getting the name from the file
      config.name = undefined;

      Mesh.create(config)

        .then(function (mesh) {
          _this.mesh = mesh;

          test.expect(mesh._mesh.config.name).to.be(meshName);
          test.expect(mesh._mesh.config.name).to.not.be(null);
          test.expect(mesh._mesh.config.name).to.not.be(undefined);

          done();
        })

        .catch(done);
    });
  });

  it('restarts the unpersisted mesh, ensures the name is different', function (done) {
    var _this = this;

    _this.unpersistedMesh.stop({ reconnect: false }, function (e) {
      if (e) return done(e);

      unpersistedConfig.name = undefined;
      unpersistedConfig.happn.name = undefined;

      Mesh.create(unpersistedConfig)

        .then(function (unpersistedMesh) {
          _this.unpersistedMesh = unpersistedMesh;

          test.expect(unpersistedMesh._mesh.config.name).to.not.be(unpersistedMeshName);
          test.expect(unpersistedMesh._mesh.config.name).to.not.be(null);
          test.expect(unpersistedMesh._mesh.config.name).to.not.be(undefined);

          test.expect(unpersistedMeshName).to.not.be(null);
          test.expect(unpersistedMeshName).to.not.be(undefined);

          done();
        })

        .catch(done);
    });
  });
});
