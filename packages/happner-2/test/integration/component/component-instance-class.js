...
require('../../__fixtures/utils/test_helper').describe({ timeout: 120e3 }, (test) => {
  ...
  it('test method is a function, test various argument scenarios', async () => {
    ...
    test.expect(await client.exchange.test.testMethodArgs(undefined, 4)).to.equal(4);
    test.expect(await client.exchange.test.testMethodArgs(undefined, undefined)).to.equal(0);
    test.expect(await client.exchange.test.testMethodArgs(null, 4)).to.equal(4);
    test.expect(await client.exchange.test.testMethodArgs(null, null)).to.equal(0);
