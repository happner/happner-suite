require('../../__fixtures/utils/test_helper').describe({ timeout: 120e3 }, (test) => {
  ...
  it('loads package.json from require', function () {
    test
      .expect(server._mesh.elements['happner-test-modules'].module.package.version)
      .to.be('1.0.2');
  });

  it('loads version from component package.json from require', function () {
    test.expect(server.describe().components['happner-test-modules'].version).to.be('1.0.2');
  });
  ...
  it('places __version onto exchange', function (done) {
    server.exchange.componentName.getVersion(function (e, version) {
      if (e) return done(e);
      try {
        test.expect(version).to.be('3.0.0');
        done();
      } catch (e) {
        done(e);
      }
  ...
  it('loads package.json from path', function () {
    test.expect(server._mesh.elements.componentName.module.package.version).to.be('3.0.0');
  });

  it('loads version from component package.json from path', function () {
    test.expect(server.describe().components.componentName.version).to.be('3.0.0');
  });
  ...
  it('includes meta componentVersion in events', function (done) {
    ...
    client.event.componentName.on('event/one', function (data, meta) {
      try {
        test.expect(meta.componentVersion).to.be('3.0.0');
