module.exports = class TestAuthProvider extends (
  require('happn-3').providers.SecurityBaseAuthProvider
) {
  ...
  async providerCredsLogin(credentials, sessionId) {
    if (
      credentials.username === 'secondTestuser@somewhere.com' &&
      credentials.password === 'secondPass'
    ) {
      let user = { username: 'secondTestuser@somewhere.com', groups: { _MESH_GST: { data: {} } } };
      return this.loginOK(credentials, user, sessionId, null, {
        myToken: 'SecretTokenValueFromAuthProvider',
      });
    }
  ...
  async providerChangePassword(user, passwordDetails, decodedToken) {
    throw new Error(`${JSON.stringify(decodedToken)}`);
  }
};
