describe(
   ...
   */
    ...
    it('the uses the onPublished event handler', function(callback) {
      listenerclient
        .on('/2_websockets_embedded_sanity/' + test_id + '/testsubscribe/data/onPublished/*', {
        ...
        .then(function(eventId) {
          ...
          expect(
            listenerclient.state.events[
              '/ALL@/2_websockets_embedded_sanity/' + test_id + '/testsubscribe/data/onPublished/*'
            ].length
          ).to.be(1);

          publisherclient.set(
            '/2_websockets_embedded_sanity/' + test_id + '/testsubscribe/data/onPublished/blah',
            {
              property1: 'property1',
              property2: 'property2',
              property3: 'property3'
    ...
    it('the listener should pick up a wildcard event, no parameters', function(callback) {
      try {
        //first listen for the change
        listenerclient.on(
          '/2_websockets_embedded_sanity/' + test_id + '/testsubscribe/data/anyevent/*',
          function() {
            expect(
              listenerclient.state.events[
                '/ALL@/2_websockets_embedded_sanity/' + test_id + '/testsubscribe/data/anyevent/*'
              ].length
            ).to.be(1);
            callback();
          },
          function(e) {
            if (!e) {
              ...
              //then make the change
              publisherclient.set(
                '/2_websockets_embedded_sanity/' + test_id + '/testsubscribe/data/anyevent/blah',
                {
                  property1: 'property1',
                  property2: 'property2',
                  property3: 'property3'
    ...
    it('the publisher should get null for unfound data, exact path', function(callback) {
      ...
      publisherclient.get(
        '2_websockets_embedded_sanity/' + test_id + '/unfound/exact/' + test_path_end,
        null,
        function(e, results) {
          expect(e).to.be(null);
    ...
    it('the publisher should get [] for unfound data, wildcard path', function(callback) {
      ...
      publisherclient.get(
        '2_websockets_embedded_sanity/' + test_id + '/unfound/wild/*' + test_path_end + '/*',
        null,
        function(e, results) {
          expect(e).to.be(null);
    ...
    it('should delete multiple items', function(callback) {
      try {
        //We put the data we want to delete into the database
        publisherclient.set(
          '/2_websockets_embedded_sanity/' + test_id + '/testsubscribe/data/delete_us/1',
          {
            property1: 'property1',
            property2: 'property2',
            property3: 'property3'
          ...
          function(e) {
            ...
            //We put the data we want to delete into the database
            publisherclient.set(
              '/2_websockets_embedded_sanity/' + test_id + '/testsubscribe/data/delete_us/2',
              {
                property1: 'property1',
                property2: 'property2',
                property3: 'property3'
              ...
              function(e) {
                ...
                publisherclient.remove(
                  '/2_websockets_embedded_sanity/' + test_id + '/testsubscribe/data/delete_us/*',
                  {
                    noPublish: true
                  },
                  function(e, result) {
    ...
    it('subscribes with a count - we ensure the event only gets kicked off for the correct amounts', function(callback) {
      var hits = 0;
      listenerclient.on(
        '/2_websockets_embedded_sanity/' + test_id + '/count/2/*',
        {
          event_type: 'set',
          count: 2
        },
    ...
    it('subscribe, then does an off with a bad handle', function(done) {
      ...
      //first listen for the change
      listenerclient.on(
        '/2_websockets_embedded_sanity/' + test_id + '/off-handle/2/*',
        {
          event_type: 'set'
        },
        function() {
    ...
    it('increments a value on a path', function(done) {
      ...
      async.timesSeries(
        ...
        function(time, timeCB) {
          publisherclient.set(test_base_url, 'counter', { increment: 1, noPublish: true }, function(
            e
          ) {
            timeCB(e);
          });
