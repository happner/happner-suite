describe(require('../__fixtures/utils/test_helper').create().testName(__filename, 3), function () {
  ...
  context('login', function () {
    ...
    //this is a bug whereby the client that fails to login does not release some resources
    //test passes but causes the test process to hang around
    xit('fails to authenticate with the _ADMIN user, using a bad password', function (done) {
      getClient(
        {
          username: '_ADMIN',
          password: 'bollocks',
